...
var hostFunctions = []apiEntry{
	...
	{"gopdf.formats.extensions()", "string[]", "Return the lower-case extensions, without a leading dot, that the linked document engine can open. The set comes from the engine, so it reflects the formats this build actually supports rather than a fixed list."},
...
var pluginFunctions = []apiEntry{
	...
	{"plugin.storage:set(key, value)", "none", "Synchronously persist a plugin-scoped value. Supports nil, booleans, finite numbers, strings, contiguous arrays, and string-keyed objects; tables cannot be cyclic or mix array and object keys. Raises an error when no session database is available, rather than discarding the value."},
...
func renderPortablePluginReference(b *strings.Builder) {
	b.WriteString("\n### Portable API\n\n")
	b.WriteString("The portable plugin surface is **API version 2**. A plugin manifest should set `\"api\": 2`; omitted or zero `api` defaults to 2, and manifests requiring a newer API are rejected. The `gopdf` tables below are available to configuration and plugin Lua code.\n\n")
